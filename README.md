...
スコープは、アクセストークンに含めることができる権限の範囲を表す単位です。
クライアントは、このスコープを指定してアクセストークンを要求します。

ここで「アクセストークンに付与される権限」と表現していますが、**スコープ自体が具体的な操作権限の意味を持つのは、 GraphなどでMicrosoftが定義しているAPIのスコープに限られます**。
...
Entra IDで認証を受ける際、Graphやバックエンドなどのリソースサーバーに登録されたスコープを指定することで、それらのスコープに対する同意をまとめて取得できます。
...
* Graph用のアクセストークンを要求する場合

```json
scopes: ["User.Read", "Mail.Read"]
```

* バックエンド用のアクセストークンを要求する場合
